...
- **`data/frequency.json`**: ~2,400 real English words, built (not hand-typed)
  from two open datasets: word frequency ranks from
  [wordfreq-en-25000](https://github.com/aparrish/wordfreq-en-25000) and a
  validity check against the
  [dwyl/english-words](https://github.com/dwyl/english-words) dictionary, so
  every entry is a real dictionary word. On top of that, entries were run
  through the app's own lemmatizer (so the keys match exactly what the
  scanning engine looks up) and passed through a manual blocklist to remove
  country/city/personal names, informal contractions (`gonna`, `dont`) and
  profanity. This filtering is good but not perfect at this scale — if you
  spot a stray name or oddity, remove it directly from the JSON file. CEFR
  levels are an approximation based on frequency rank buckets, not a
  linguist-reviewed classification. See `data/frequency.json`'s structure for
  the shape if you want to regenerate it with a larger source list.
- **`data/dictionary.json`**: a small fallback dictionary (layer 2 of the
  meaning lookup), covering the demo words plus a starter set. Replace with a
  WordNet/Wiktionary extract for wider coverage.
